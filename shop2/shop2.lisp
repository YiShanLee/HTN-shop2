...
(defun get-domain-knowledge (domain-file problem-file)
  (setq *domain* (read-hddl-domain domain-file)
   *problem* (read-hddl-problem problem-file))
  )
;--------------------------------------------
;; main method for first layer of shop2
;; todo backtrack to this method with depth from failed action step and failed method step
(defun shop2-operator(depth Tasks T0 Plan methods actions current-state substitution)
  (cond (null depth)
  (let* ((Tasks (hddl-problem-task *problem*))
...
(defun resolve-task (domain problem current-state Plan methods actions T0 current-task substitution Tasks) 
  (if (primitivep current-task)
    ...
    ; non-primitive (nil)
    (update-nonprimitive-task methods current-task substitution Tasks current-state T0) ;todo handle return value from function
    )
)
   ...
   (let* ((unifying_actions (action-unifier actions current-task))
          (Actions-lst (action-satisfyp unifying_actions current-state)))
                      (cond ((eq Actions-lst nil) (return-from (update-primitive-task nil)))
                                         (t (update-action-values Actions-lst current-state Plan Tasks substitution))
                                         )
                      (setq T0 constraint(Tasks)))
   )
 (defun update-action-values (Action-list current-state Plan Tasks substitution)
   (let* ((act (car Action-lst)) 
          (setq current-state (modify-status current-state act)) 
          (push (car act) Plan)
          (setq Tasks (remove-task current-task Tasks)) ;TODO function ;;Alisa: zusätzlich müssten wir dann auch schauen, dass wir die task aus allen task constraint-Listen löschen!
          (substitution (substitute (act current-task))))) 
   )
 ;; unify action and update state from nonprimitive task
 (defun update-nonprimitive-task (methods current-task substitution Tasks current-state T0)
   (let (Methods-lst (method-unifier methods current-task current-state)) 
                       ; if M = empty then return failure
                     (cond ((eq Methods-lst nil) (return nil))
                  ; nondeterministically choose a pair (m, θ) ∈ M
                               (t (update-method-values Methods-lst substitution T0) 
                                       )))
   )
 (defun update-method-values (Methods-lst substitution T0)
