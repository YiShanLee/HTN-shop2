...
language HDDL.
The files are analysed and returned as structures with the appropriate components.
The components of the  returned domain or problem can themselves contain other structures or lists of structures.

The package exports the main reading functions as well as access functions and constructors to the defined HDDL-structures.
|#
...
(defun ordered-subtasks-p (methods)
  ...
  (let ((newmethods)
      ...
      (loop for (x name y parameters z task w subtasks) in methods do
	       ;;if the string before the subtasks contains the word ordered (:ORDERED-SUBTASKS) then set ordered to true, else to nil
		  (if (search "ORDERED" (string w))       
			  (setq ordered t)
		      (setq ordered nil))
		  ;;make HDDL-method and push to new methodslist
		  (push (make-hddl-method :name name :parameters (typing parameters) :task task :subtasks subtasks :ordered-subtasks ordered) newmethods))
      (reverse newmethods)))
...
;;------------------------------------------------------------------------------------- 
;;-------------------------------------------------------------------------------------
;; Main functions for reading in a domain.hddl and problem.hddl-file
  ...
  "Reads in a HDDL-domain-file and transforms it into a usable HDDL-structure."
    ...
    (let* ((read-domain (read-file filename))
		...
		(let ((hddl-requirements)
			...
			(setq hddl-requirements (cdr (car requirements))                       ;remove unneccessary key-word from beginning of requirements-list
			
				  hddl-types (typing (cdr (car types)))                            ;transform types into a list of lists: (subtype type)
				  ...
				  hddl-actions                                                     ;then use that action-list with separated effects to build hddl-actions
					(loop for (x name y parameters z preconditions w neg-effects pos-effects) in temp-actions collect
						(make-hddl-action :name name :parameters (typing parameters) :preconditions (remove-and preconditions) 
							:neg-effects  neg-effects :pos-effects pos-effects))
...
;;Reads in a HDDL-problem-file and transforms it into a usable HDDL-problem structure.
...
(defun read-hddl-problem (filename)
  "Reads in a HDDL-problem-file and transforms it into a usable HDDL-problem structure."
	...
	(let* ((read-problem (read-file filename))
		...
		(declare (special objects htn status))                       ;lexical declaration of variables for matching of defined lists and elements in look-up
		...
		(let ((tasks)
		    ...
		    (let ((hddl-tasks)
				...
				(setq hddl-tasks
					  ...
					  problem (make-hddl-problem :name (second read-problem) :domain (cdr (third read-problem)) :objects (typing (cdr (car objects))) 
								:tasks hddl-tasks :ordering ordering :constraints constraints :init-status (cdr (car status))))))))
